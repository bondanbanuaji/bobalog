...
function createTextTexture(
  ...
  canvas.width = (textWidth + 20) * 4;
  canvas.height = (textHeight + 20) * 4;
  ...
  const texture = new Texture(gl, { generateMipmaps: true, premultiplyAlpha: true });
...
class App {
  ...
  createRenderer() {
    this.renderer = new Renderer({
      alpha: true,
      antialias: true,
      dpr: window.devicePixelRatio ? Math.max(window.devicePixelRatio, 2) : 2
    });
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    if (Date.now() - this.downTime < 300) {
      ...
      if (Math.abs(this.start - clientX) < 10 && this.onItemClick && this.medias && this.medias.length > 0) {
        // Simple hit testing based on screen X relative to center
        const width = this.medias[0].width;
        let itemIndex = Math.round(Math.abs(this.scroll.target) / width);
        
        const screenCenter = this.screen.width / 2;
        const clickOffset = clientX - screenCenter;
        
        // Approximate width on screen for hit test calculation
        const screenItemWidth = this.screen.width * 0.4;
        const offsetIndex = Math.round(clickOffset / screenItemWidth);
        
        const clickedIndex = itemIndex + offsetIndex;
        const actualIndex = ((clickedIndex % this.mediasImages.length) + this.mediasImages.length) % this.mediasImages.length;
